...
1. **MCP (Cursor)** — `.cursor/mcp.json` → `x-api-key` header (allaqachon o'rnatilgan).
2. **CLI** — muhit o'zgaruvchisi yoki flag:
   - `$env:API_KEY_21ST = "21st_sk_870e6dc67baeddfa09b09e2e4aab53d9622abe5fd1789c08cd7777e05c472bf1"`
   - yoki `npx @21st-dev/cli --api-key 21st_sk_... search "button"`

Alternativa: `npx @21st-dev/cli login` (brauzer orqali).

`21st whoami` — hisob tekshirish; `21st usage` — kvota.
