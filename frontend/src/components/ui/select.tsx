...
/**
 * Base UI's `Select.Value` only shows a translated/custom label when an
 * `items` map is passed to `Select.Root` — otherwise it falls back to the
 * raw `value` (e.g. "income" instead of "Kirim"). Every call site in this
 * app authors `<SelectItem value=".."> label </SelectItem>` the "Radix way"
 * without supplying `items`, so we derive that map automatically by
 * walking the authored children tree for `SelectItem` elements.
 */
...
function Select({
  ...
  className,
  ...props
}: SelectPrimitive.Root.Props<any> & { className?: string }) {
...
function SelectTrigger({
  ...
  /** Standart pastga qaragan chevron o'rniga ko'rsatiladigan belgi (masalan tahrirlash qalamchasi). */
...
function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "relative flex w-full cursor-default items-center gap-2 rounded-lg py-2 pr-8 pl-2.5 text-sm outline-hidden transition-colors select-none hover:bg-muted/70 focus:bg-accent focus:text-accent-foreground not-data-[variant=destructive]:focus:**:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
        className
      )}
      {...props}
    >
