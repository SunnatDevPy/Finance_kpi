...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "h-12 px-5 text-left align-middle text-[11px] font-semibold tracking-[0.07em] uppercase whitespace-nowrap text-muted-foreground",
...
function TableCell({ className, ...props }: React.ComponentProps<"td">) {
  return (
    <td
      ...
      className={cn(
        "px-5 py-4 align-middle text-sm leading-relaxed",
        "[&:has([role=checkbox])]:pr-0 [&>[role=checkbox]]:translate-y-[2px]",
        className,
      )}
      {...props}
